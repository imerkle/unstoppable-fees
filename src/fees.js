...
async function getTxs(address) {
    ...
    var txsOut = txs.filter(v => v.from === address.toLowerCase());
    /*
    ...
    */
    ...
    var txsOutFail = txsOut.filter(v => v.isError === '1');
    ...
    var gasUsedTotalStr;
    var gasPricePerTxStr;
    var gasFeeTotalStr;
    var tokenusdStr;
    var oofCostStr;
    var gasFeeTotalFailStr;
    if (nOut > 0) {
        ...
        gasUsedTotalStr = comma(formatter(gasUsedTotal))
        //  $('#gasUsedTotal').text(comma(formatter(gasUsedTotal)));
        gasPricePerTxStr = comma((gasPriceTotal / nOut / 1e9).toFixed(1))
        ...
        */
        ...
        gasFeeTotalStr = chainConfig[chainId].token + comma((gasFeeTotal / 1e18).toFixed(3))

        if (nOutFail > 0) {
            //$('#gasFeeTotalFail').html(chainConfig[chainId].token + (gasFeeTotalFail / 1e18).toFixed(3));
            gasFeeTotalFailStr = chainConfig[chainId].token + (gasFeeTotalFail / 1e18).toFixed(3)
        ...
        if (tokenusd !== null) {
            ...
            tokenusdStr = '$' + comma(formatter((tokenusd * gasFeeTotal / 1e18).toFixed(2)))
            oofCostStr = ' ($' + comma(formatter((tokenusd * gasFeeFail[i] / 1e18).toFixed(2))) + ')'
        }
    } else {
        gasUsedTotalStr = 0
        gasFeeTotalStr = chainConfig[chainId].token + 0
        //$('#gasUsedTotal').text(0);
